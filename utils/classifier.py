...
def train_classifier(data_dir, export_dir, batch_size=8):
    facenet_path = os.path.join(os.curdir, 'Model', 'FaceRecog', 'FaceNet_Keras_converted.h5')
    # create a Face Recognizer object
    faceNet = Recognizer(facenet_path)

    # Create input pipeline
    datagen = ImageDataGenerator(preprocessing_function=faceNet.preprocess)
    ...
    face_embeddings = faceNet.net.predict(train_gen, steps=1+train_gen.samples//batch_size )
    ...
    svc_clf = SVC(kernel='rbf', probability=True, verbose=True)
